...
class MinisApp : Application(), ImageLoaderFactory {
    ...
    lateinit var memoryRepository: MemoryRepository
        private set
    lateinit var subAgentRepository: SubAgentRepository
        private set
    ...
    override fun onCreate() {
        ...
        subAgentRepository = SubAgentRepository(this)
    ...
    /**
     * [T-deepseek-auto-setup] On first launch, automatically create a DeepSeek
     * provider instance using the API key from BuildConfig (read from .env at
     * build time). The key is stored in EncryptedSharedPreferences and is never
     * visible or copyable from the app UI.
     *
     * Runs on the IO dispatcher and awaits the async provider-config load so
     * the "already exists" check sees the persisted config (prevents duplicate
     * instances on subsequent launches). addInstance() itself is thread-safe
     * (guarded by ProviderRepository.configLock).
     */
    private fun autoSetupDeepSeekProvider() {
        val apiKey = BuildConfig.DEEPSEEK_API_KEY
        if (apiKey.isBlank()) {
            Log.w("MinisApp", "DEEPSEEK_API_KEY is empty — skipping auto-setup")
            return
        }
        kotlinx.coroutines.CoroutineScope(
            ...
            // Check if a DeepSeek instance already exists (by customBaseURL)
            val alreadyExists = providerRepository.instances.any { instance ->
                instance.customBaseURL?.lowercase()?.contains("api.deepseek.com") == true
            }
            if (alreadyExists) {
                Log.i("MinisApp", "DeepSeek provider already exists — skipping auto-setup")
                return@launch
            }
            ...
            Log.i("MinisApp", "Auto-created DeepSeek provider instance: ${instance.id}")
        }
    }
