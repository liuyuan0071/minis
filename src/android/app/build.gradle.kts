...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 35
        versionCode = 21
        versionName = "0.21-fix2"

        testInstrumentationRunner = "androidx.test.runner.AndroidJUnitRunner"

        // [T-deepseek-auto-setup] Read DEEPSEEK_API_KEY from the root .env file
        // at build time and inject it into BuildConfig so MinisApp can auto-create
        // a DeepSeek provider on first launch. The .env file is NOT bundled into
        // the APK — only the key value is captured.
        val envFile = rootProject.file("../../.env")
        val deepseekKeyRaw = if (envFile.exists()) {
            envFile.readLines()
        ...
        // Escape so an unusual key value can't break the generated BuildConfig literal.
        val deepseekKey = deepseekKeyRaw
            .replace("\\", "\\\\")
            .replace("\"", "\\\"")
            .replace("$", "\\$")
